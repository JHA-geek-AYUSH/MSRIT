...
# GemmaFinOS Phase 2 - Validator Setup Script
...
check_subnet_exists() {
    ...
    if ! gemmachain subnet list | grep -q "gemmaFin-private"; then
        error "Subnet 'gemmaFin-private' not found. Run 1-setup-subnet.sh first."
    fi
...
FROM avaplatform/gemmachaingo:latest
...
# Create directories
RUN mkdir -p /opt/gemmachain/configs /opt/gemmachain/keys /opt/gemmachain/logs \
    && chown -R validator:validator /opt/gemmachain

# Copy configuration files
COPY configs/ /opt/gemmachain/configs/
COPY keys/ /opt/gemmachain/keys/

# Switch to validator user
USER validator
WORKDIR /opt/gemmachain
...
# Default command
CMD ["/gemmachaingo/build/gemmachaingo", \
     "--config-file=/opt/gemmachain/configs/node.json", \
     "--log-level=info", \
     "--log-dir=/opt/gemmachain/logs"]
EOF
...
services:
  validator-1:
    ...
    container_name: gemmaFin-validator-1
    ...
    ports:
      ...
      - "8545:8545"  # RPC port
    volumes:
      - validator-1-data:/opt/gemmachain/data
      - ./configs/validator-1:/opt/gemmachain/configs:ro
      - ./keys/validator-1:/opt/gemmachain/keys:ro
      - validator-1-logs:/opt/gemmachain/logs
    environment:
      - VALIDATOR_ID=1
      - NODE_ID_FILE=/opt/gemmachain/keys/node.id
    networks:
      - gemmaFin-private
    logging:
      driver: "json-file"
  ...
  validator-2:
    build:
      context: .
      dockerfile: Dockerfile.validator
    container_name: gemmaFin-validator-2
    restart: unless-stopped
    ports:
      ...
      - "8546:8545"  # RPC port
    volumes:
      - validator-2-data:/opt/gemmachain/data
      - ./configs/validator-2:/opt/gemmachain/configs:ro
      - ./keys/validator-2:/opt/gemmachain/keys:ro
      - validator-2-logs:/opt/gemmachain/logs
    environment:
      - VALIDATOR_ID=2
      - NODE_ID_FILE=/opt/gemmachain/keys/node.id
    networks:
  ...
  validator-3:
    build:
      context: .
      dockerfile: Dockerfile.validator
    container_name: gemmaFin-validator-3
    restart: unless-stopped
    ports:
      ...
      - "8547:8545"  # RPC port
    volumes:
      - validator-3-data:/opt/gemmachain/data
      - ./configs/validator-3:/opt/gemmachain/configs:ro
      - ./keys/validator-3:/opt/gemmachain/keys:ro
      - validator-3-logs:/opt/gemmachain/logs
    environment:
      - VALIDATOR_ID=3
      - NODE_ID_FILE=/opt/gemmachain/keys/node.id
    networks:
        ...
        cat > "$config_dir/node.json" << EOF
{
    ...
    "log-level": "info",
    "log-dir": "/opt/gemmachain/logs",
    "db-dir": "/opt/gemmachain/data/db",
    "http-host": "0.0.0.0",
    "http-port": 9650,
    "staking-port": 9651,
    "public-ip": "",
    "staking-enabled": true,
    "staking-tls-cert-file": "/opt/gemmachain/keys/staker.crt",
    "staking-tls-key-file": "/opt/gemmachain/keys/staker.key",
    "bootstrap-ips": "",
    "bootstrap-ids": "",
    ...
    "http-tls-cert-file": "/opt/gemmachain/keys/http.crt",
    "http-tls-key-file": "/opt/gemmachain/keys/http.key",
    "snow-sample-size": 20,
    "snow-quorum-size": 15,
...
generate_tls_certificates() {
    ...
    for i in {1..3}; do
        ...
        openssl req -new -key "$cert_dir/staker.key" -out "$cert_dir/staker.csr" -subj "/CN=gemmaFin-validator-$i.internal"
        openssl req -new -key "$cert_dir/http.key" -out "$cert_dir/http.csr" -subj "/CN=gemmaFin-validator-$i.internal"
  ...
  - name: gemmachain.rules
    rules:
      ...
      - alert: SubnetUnhealthy
        expr: gemmachain_health_checks_passing{job="subnet-health"} == 0
        for: 2m
        labels:
          severity: critical
        annotations:
...
# GemmaFinOS Validator Deployment Status
...
1. Test subnet connectivity: \`gemmachain subnet status gemmaFin-private --local\`
...
# View validator logs
docker logs gemmaFin-validator-1
