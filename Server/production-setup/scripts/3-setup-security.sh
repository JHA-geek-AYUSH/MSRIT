...
# GemmaFinOS Phase 2 - Security Setup Script
...
terraform {
  ...
  backend "s3" {
    bucket = "gemmaFin-terraform-state"
...
resource "aws_vpc" "gemmaFin_vpc" {
  cidr_block           = "10.0.0.0/16"
  enable_dns_hostnames = true
  enable_dns_support   = true
  
  tags = {
    Name = "gemmaFin-private-vpc"
  }
}

# Internet Gateway (for NAT Gateway)
resource "aws_internet_gateway" "gemmaFin_igw" {
  vpc_id = aws_vpc.gemmaFin_vpc.id
...
resource "aws_subnet" "private_subnets" {
  ...
  tags = {
    Name = "gemmaFin-private-subnet-${count.index + 1}"
    Type = "private"
  }
}
...
resource "aws_eip" "nat_eip" {
  ...
  depends_on = [aws_internet_gateway.gemmaFin_igw]
  
  tags = {
    Name = "gemmaFin-nat-eip"
  }
}

# NAT Gateway
resource "aws_nat_gateway" "gemmaFin_nat" {
...
resource "aws_route_table" "public_rt" {
  ...
  route {
    ...
    gateway_id = aws_internet_gateway.gemmaFin_igw.id
  }
  
  tags = {
    Name = "gemmaFin-public-rt"
  }
}
...
resource "aws_route_table" "private_rt" {
  ...
  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.gemmaFin_nat.id
  }
  
  tags = {
    Name = "gemmaFin-private-rt"
  }
}
...
resource "aws_security_group" "validator_sg" {
  name_prefix = "gemmaFin-validator-"
  vpc_id      = aws_vpc.gemmaFin_vpc.id
  description = "Security group for GemmaFinOS validators"
  
  # GemmaChain staking port (between validators)
  ingress {
    from_port   = 9651
    to_port     = 9651
    protocol    = "tcp"
  ...
  # GemmaChain API port (from VPN subnet)
  ingress {
    from_port   = 9650
    to_port     = 9650
    protocol    = "tcp"
  ...
  tags = {
    Name = "gemmaFin-validator-sg"
  }
}

# Security group for VPN endpoint
resource "aws_security_group" "vpn_sg" {
  name_prefix = "gemmaFin-vpn-"
  vpc_id      = aws_vpc.gemmaFin_vpc.id
  description = "Security group for GemmaFinOS VPN endpoint"
...
resource "aws_subnet" "vpn_subnet" {
  ...
  tags = {
    Name = "gemmaFin-vpn-subnet"
  }
}

# Client VPN endpoint
resource "aws_ec2_client_vpn_endpoint" "gemmaFin_vpn" {
  description            = "GemmaFinOS Private Subnet VPN"
...
resource "aws_ec2_client_vpn_network_association" "vpn_association" {
  ...
  client_vpn_endpoint_id = aws_ec2_client_vpn_endpoint.gemmaFin_vpn.id
  subnet_id              = aws_subnet.private_subnets[count.index].id
}
...
resource "aws_ec2_client_vpn_authorization_rule" "vpn_auth" {
  client_vpn_endpoint_id = aws_ec2_client_vpn_endpoint.gemmaFin_vpn.id
  target_network_cidr    = aws_vpc.gemmaFin_vpc.cidr_block
  authorize_all_groups   = true
}

# CloudWatch log group for VPN
resource "aws_cloudwatch_log_group" "vpn_logs" {
  name              = "/aws/clientvpn/gemmaFin"
...
output "vpn_endpoint_id" {
  ...
  value       = aws_ec2_client_vpn_endpoint.gemmaFin_vpn.id
}

output "vpn_dns_name" {
  description = "VPN endpoint DNS name"
  value       = aws_ec2_client_vpn_endpoint.gemmaFin_vpn.dns_name
}
EOF
...
generate_vpn_certificates() {
    ...
    openssl req -new -x509 -days 3650 -key ca.key -out ca.crt -subj "/CN=GemmaFinOS-VPN-CA"
    
    # Generate server private key
    openssl genrsa -out server.key 4096
    
    # Generate server certificate request
    openssl req -new -key server.key -out server.csr -subj "/CN=gemmaFin-vpn.internal"
    ...
    openssl req -new -key client.key -out client.csr -subj "/CN=gemmaFin-client"
...
create_mtls_config() {
    ...
    openssl req -new -x509 -days 3650 -key mtls-ca.key -out mtls-ca.crt -subj "/CN=GemmaFinOS-mTLS-CA"
    
    # Server certificate for RPC
    openssl genrsa -out rpc-server.key 4096
    openssl req -new -key rpc-server.key -out rpc-server.csr -subj "/CN=gemmaFin-rpc.internal"
    ...
    openssl req -new -key backend-client.key -out backend-client.csr -subj "/CN=gemmaFin-backend"
...
server {
    listen 443 ssl;
    server_name gemmaFin-rpc.internal;
...
services:
  mtls-proxy:
    image: nginx:alpine
    container_name: gemmaFin-mtls-proxy
...
# GemmaFinOS Subnet Firewall Rules
...
# Allow GemmaChain staking port from private network
iptables -A INPUT -p tcp --dport 9651 -s 10.0.0.0/16 -j ACCEPT

# Allow GemmaChain API port from VPN subnet
...
create_vpn_client_config() {
    ...
    cat > "$client_dir/gemmaFin-client.ovpn" << EOF
client
dev tun
proto udp
remote $vpn_dns 1194
...
EOF
    ...
    log "   Config file: $client_dir/gemmaFin-client.ovpn"
}
...
# GemmaFinOS Security Configuration Summary
...
2. Import configuration: \`vpn-client/gemmaFin-client.ovpn\`
3. Connect to VPN
4. Access RPC via: \`https://gemmaFin-rpc.internal\`
