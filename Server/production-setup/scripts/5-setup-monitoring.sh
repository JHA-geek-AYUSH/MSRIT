...
# GemmaFinOS Phase 2 - Monitoring and Alerting Setup Script
...
scrape_configs:
  # GemmaChain validators
  - job_name: 'gemmachain-validators'
    static_configs:
      - targets: 
          - 'validator-1:9650'
          - 'validator-2:9650'
...
groups:
  - name: gemmachain.rules
    rules:
      - alert: ValidatorDown
        expr: up{job="gemmachain-validators"} == 0
        for: 1m
        labels:
          severity: critical
          component: validator
        annotations:
          summary: "GemmaChain validator {{ $labels.instance }} is down"
          description: "Validator {{ $labels.instance }} has been down for more than 1 minute."
          runbook_url: "https://docs.gemmaFin.com/runbooks/validator-down"

      - alert: ValidatorNotSyncing
        expr: gemmachain_network_is_bootstrapped{job="gemmachain-validators"} == 0
        for: 5m
        labels:
          severity: critical
          component: validator
...
groups:
  - name: gemmaFin.recording.rules
    interval: 30s
    rules:
      - record: gemmaFin:transaction_rate_5m
        expr: rate(gemmachain_transaction_count_total[5m])

      - record: gemmaFin:block_production_rate_5m
        expr: rate(gemmachain_block_count_total[5m])

      - record: gemmaFin:contract_gas_usage_avg_5m
        expr: avg_over_time(contract_gas_used[5m])

      - record: gemmaFin:validator_uptime_percentage
        expr: avg_over_time(up{job="gemmachain-validators"}[1h]) * 100

      - record: gemmaFin:network_latency_p99_5m
        expr: histogram_quantile(0.99, rate(network_latency_seconds_bucket[5m]))

      - record: gemmaFin:encryption_operations_rate_5m
        expr: rate(encryption_operations_total[5m])

      - record: gemmaFin:rpc_request_rate_5m
        expr: rate(rpc_requests_total[5m])

      - record: gemmaFin:error_rate_5m
...
create_grafana_dashboards() {
    ...
    # Main GemmaFinOS Overview dashboard
    cat > "$MONITORING_DIR/dashboards/gemmaFin-overview.json" << 'EOF'
{
  "dashboard": {
    "id": null,
    "title": "GemmaFinOS Subnet Overview",
    "description": "High-level overview of GemmaFinOS private subnet health and performance",
    "tags": ["gemmaFin", "gemmachain", "subnet"],
    "timezone": "UTC",
    "refresh": "30s",
    "time": {
      "from": "now-1h",
    ...
    "panels": [
      {
        ...
        "targets": [
          {
            "expr": "up{job=\"gemmachain-validators\"}",
      ...
      },
      {
        ...
        "targets": [
          {
            "expr": "gemmaFin:transaction_rate_5m",
      ...
      },
      {
        ...
        "targets": [
          {
            "expr": "gemmaFin:block_production_rate_5m",
      ...
      },
      {
        ...
        "targets": [
          {
            "expr": "gemmaFin:network_latency_p99_5m",
    ...
    cat > "$MONITORING_DIR/dashboards/security-dashboard.json" << 'EOF'
{
  "dashboard": {
    "id": null,
    "title": "GemmaFinOS Security Dashboard",
    "description": "Security monitoring for GemmaFinOS private subnet",
    "tags": ["gemmaFin", "security", "encryption"],
    "timezone": "UTC",
    "refresh": "10s",
    "panels": [
      {
      ...
      },
      {
        ...
        "targets": [
          {
            "expr": "gemmaFin:encryption_operations_rate_5m",
...
global:
  ...
  smtp_from: 'gemmaFin-alerts@company.com'
...
receivers:
  - name: 'default'
    email_configs:
      - to: 'team@company.com'
        subject: 'GemmaFinOS Alert: {{ .GroupLabels.alertname }}'
        body: |
          {{ range .Alerts }}
  ...
  - name: 'critical-alerts'
    email_configs:
      - to: 'critical@company.com'
        subject: 'CRITICAL GemmaFinOS Alert: {{ .GroupLabels.alertname }}'
        body: |
          CRITICAL ALERT for GemmaFinOS Production Subnet
  ...
  - name: 'security-team'
    email_configs:
      - to: 'security@company.com'
        subject: 'GemmaFinOS Security Alert: {{ .GroupLabels.alertname }}'
    slack_configs:
      - api_url: 'YOUR_SLACK_WEBHOOK_URL'
        channel: '#gemmaFin-security'

  - name: 'ops-team'
    email_configs:
      - to: 'ops@company.com'
        subject: 'GemmaFinOS Operations Alert: {{ .GroupLabels.alertname }}'
...
[OUTPUT]
    ...
    Match             *
    region            ap-south-1
    log_group_name    /gemmaFin/production
...
"""
Contract Monitor Service
Monitors GemmaFinOS smart contracts for events and metrics
"""
...
class SecurityMonitor:
    ...
    def check_kms_keys(self):
        ...
        key_aliases = [
            'alias/gemmaFin-subnet-admin',
            'alias/gemmaFin-contract-deployer',
            'alias/gemmaFin-backend-tx',
            'alias/gemmaFin-treasury'
        ]
...
services:
  prometheus:
    ...
    container_name: gemmaFin-prometheus
  ...
  grafana:
    ...
    container_name: gemmaFin-grafana
    restart: unless-stopped
    environment:
      - GF_SECURITY_ADMIN_PASSWORD=gemmaFin_admin_2026
  ...
  alertmanager:
    ...
    container_name: gemmaFin-alertmanager
    restart: unless-stopped
    command:
  ...
  node-exporter:
    ...
    container_name: gemmaFin-node-exporter
    restart: unless-stopped
    command:
  ...
  fluent-bit:
    ...
    container_name: gemmaFin-fluent-bit
    restart: unless-stopped
    volumes:
  ...
  contract-monitor:
    ...
    container_name: gemmaFin-contract-monitor
    restart: unless-stopped
    ports:
      - "9091:9091"
    volumes:
      - ../../backend:/opt/gemmaFin:ro
      - ./contract-monitor.py:/app/monitor.py:ro
    networks:
      - gemmaFin-monitoring
    environment:
      - PYTHONPATH=/opt/gemmaFin
  ...
  security-monitor:
    build:
      context: .
      dockerfile: Dockerfile.monitor
    container_name: gemmaFin-security-monitor
    restart: unless-stopped
    ports:
      - "9092:9092"
    volumes:
...
setup_cloudwatch_integration() {
    ...
    aws logs create-log-group --log-group-name "/gemmaFin/production/validators" --region ap-south-1 2>/dev/null || true
    aws logs create-log-group --log-group-name "/gemmaFin/production/contracts" --region ap-south-1 2>/dev/null || true
    aws logs create-log-group --log-group-name "/gemmaFin/production/security" --region ap-south-1 2>/dev/null || true
    aws logs create-log-group --log-group-name "/gemmaFin/production/application" --region ap-south-1 2>/dev/null || true
    
    # Create CloudWatch alarms
    aws cloudwatch put-metric-alarm \
        --alarm-name "GemmaFinOS-ValidatorDown" \
        --alarm-description "GemmaFinOS validator is down" \
        --metric-name "ValidatorStatus" \
        --namespace "GemmaFinOS/Subnet" \
        --statistic Sum \
        --period 300 \
        --threshold 1 \
        --comparison-operator LessThanThreshold \
        --evaluation-periods 1 \
        --alarm-actions "arn:aws:sns:ap-south-1:ACCOUNT:gemmaFin-critical-alerts" \
        --region ap-south-1 2>/dev/null || true
    
    aws cloudwatch put-metric-alarm \
        --alarm-name "GemmaFinOS-HighErrorRate" \
        --alarm-description "GemmaFinOS error rate is high" \
        --metric-name "ErrorRate" \
        --namespace "GemmaFinOS/Application" \
        --statistic Average \
        --period 300 \
        --threshold 0.05 \
        --comparison-operator GreaterThanThreshold \
        --evaluation-periods 2 \
        --alarm-actions "arn:aws:sns:ap-south-1:ACCOUNT:gemmaFin-alerts" \
        --region ap-south-1 2>/dev/null || true
...
# GemmaFinOS Monitoring and Alerting Complete
...
- **Slack**: #gemmaFin-critical, #gemmaFin-security, #gemmaFin-ops
- **CloudWatch**: AWS SNS integration for escalation

## Dashboards

### 1. GemmaFinOS Overview Dashboard
...
### Log Sources
- Validator nodes (GemmaChain logs)
...
docker logs gemmaFin-alertmanager
\`\`\`
...
**🎉 GemmaFinOS monitoring and alerting is fully operational!**
...
# Main execution
main() {
    log "📊 Starting GemmaFinOS Monitoring and Alerting Setup..."
    ...
    log "   Grafana: http://localhost:3000 (admin/gemmaFin_admin_2026)"
    log "   Prometheus: http://localhost:9090"
    log "   AlertManager: http://localhost:9093"
    log ""
    log "🚀 GemmaFinOS Phase 2 production deployment is COMPLETE!"
}
