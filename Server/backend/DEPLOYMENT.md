# GemmaFinOS Server Backend Deployment Guide

## Overview

The GemmaFinOS Server backend is a FastAPI application that provides:
- ✅ **Hardcoded Authentication** (`gemmaFinteam321` / `weareteamalphaq`)
- 🔗 **Blockchain Operations** (GemmaChain Subnet integration)
...
1. **Build and run**:
```bash
docker build -t gemmaFin-server .
docker run -p 8001:8001 gemmaFin-server
```
