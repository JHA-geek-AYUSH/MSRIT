...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  useEffect(() => {
    ...
    const savedToken = localStorage.getItem('gemmaFin_auth_token')
    if (savedToken) {
      setToken(savedToken)
  ...
  const validateToken = async (authToken: string) => {
    try {
      ...
      localStorage.removeItem('gemmaFin_auth_token')
  ...
  const login = async (userId: string, password: string): Promise<boolean> => {
    ...
    try {
      ...
      localStorage.setItem('gemmaFin_auth_token', response.access_token)
