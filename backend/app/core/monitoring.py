...
REQUEST_COUNT = Counter(
    'gemmaFin_http_requests_total',
    'Total HTTP requests',
    ['method', 'endpoint', 'status_code']
)

REQUEST_DURATION = Histogram(
    'gemmaFin_http_request_duration_seconds',
    'HTTP request duration in seconds',
    ['method', 'endpoint']
)

ACTIVE_CONNECTIONS = Gauge(
    'gemmaFin_active_connections',
    'Number of active connections'
)

AGENT_EXECUTION_TIME = Histogram(
    'gemmaFin_agent_execution_seconds',
    'Agent execution time in seconds',
    ['agent_name']
)

AGENT_SUCCESS_RATE = Counter(
    'gemmaFin_agent_executions_total',
    'Total agent executions',
    ['agent_name', 'status']
)

RETRIEVAL_OPERATIONS = Counter(
    'gemmaFin_retrieval_operations_total',
    'Total retrieval operations',
    ['operation_type', 'status']
)

VERIFICATION_RESULTS = Counter(
    'gemmaFin_verification_results_total',
    'Verification results',
    ['check_type', 'result']
)

BILLING_OPERATIONS = Counter(
    'gemmaFin_billing_operations_total',
    'Billing operations',
    ['operation_type', 'status']
)

DATABASE_OPERATIONS = Histogram(
    'gemmaFin_database_operation_duration_seconds',
...
class HealthChecker:
    ...
    async def check_database_health(self) -> Dict[str, Any]:
        """Check database connectivity and performance (works with SQLite and PostgreSQL)"""
        try:
            from app.db.session import SessionLocal, ASYNC_DATABASE_URL
            ...
            async with SessionLocal() as session:
                ...
                # Long-running queries check (PostgreSQL only)
                ...
                if "postgresql" in ASYNC_DATABASE_URL or "asyncpg" in ASYNC_DATABASE_URL:
                    try:
                        ...
                        pass  # pg_stat_activity might not be accessible
    ...
    async def check_qdrant_health(self) -> Dict[str, Any]:
        """Check Qdrant vector database health (graceful if unavailable)"""
        try:
            ...
            return {"status": "unconfigured", "error": "Qdrant module not available", "timestamp": time.time()}
        except Exception as e:
            log.warning("qdrant_health_unavailable", error=str(e))
            return {"status": "unavailable", "error": str(e), "timestamp": time.time()}
    
    async def check_redis_health(self) -> Dict[str, Any]:
        """Check Redis health (graceful if unavailable)"""
        try:
            ...
            if redis_client is None:
                return {"status": "unconfigured", "error": "Redis unavailable", "timestamp": time.time()}
            redis_client.ping()
            ...
            log.warning("redis_health_unavailable", error=str(e))
            return {"status": "unavailable", "error": str(e), "timestamp": time.time()}
    ...
    async def check_supabase_health(self) -> Dict[str, Any]:
        """Check Supabase storage health (graceful if unavailable)"""
        try:
            ...
            return {"status": "unconfigured", "error": "Supabase module not available", "timestamp": time.time()}
        except Exception as e:
            log.warning("supabase_health_unavailable", error=str(e))
            return {"status": "unavailable", "error": str(e), "timestamp": time.time()}
