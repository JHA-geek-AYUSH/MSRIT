...
async def resolve_db_user_id(db: AsyncSession, clerk_id: str, email: str = "") -> uuid.UUID:
    """
    Given a Clerk user ID (e.g. 'user_xxx'), return the internal DB UUID.
    Auto-creates user + billing account on first login.
    """
    db_user = await get_user_by_clerk_id(db, clerk_id)
    if not db_user:
        db_user = await create_user(db, clerk_id=clerk_id, email=email, role="lawyer")
        await get_or_create_billing_account(db, db_user.id)
    return db_user.id
...
async def get_matters_by_user(db: AsyncSession, user_id: uuid.UUID) -> List[Matter]:
...
async def get_documents_by_matter(db: AsyncSession, matter_id: uuid.UUID) -> List[Document]:
    """Get all documents for a matter"""
    res = await db.execute(
        select(Document).where(Document.matter_id == matter_id).order_by(Document.created_at.desc())
    )
    return list(res.scalars().all())


async def get_queries_by_matter(db: AsyncSession, matter_id: uuid.UUID) -> List[dict]:
    """Get all historical queries and runs for a matter, returned as chat history."""
    from sqlalchemy import text as sql_text
    rows = await db.execute(sql_text("""
        SELECT q.id, q.message, q.mode, q.created_at,
               r.id AS run_id, r.answer_text, r.created_at AS run_at
        FROM queries q
        LEFT JOIN runs r ON r.query_id = q.id
        WHERE q.matter_id = :mid
...
async def delete_queries_by_matter(db: AsyncSession, matter_id: uuid.UUID) -> bool:
    """Clear conversation history for a matter"""
    res = await db.execute(select(Query).where(Query.matter_id == matter_id))
