...
async def _llm_rerank(query: str, chunks: List[Dict[str, Any]], limit: int) -> List[Dict[str, Any]]:
    ...
    if client is None:
        return sorted(chunks, key=lambda chunk: chunk.get("normalized_score", 0), reverse=True)[:limit]
...
async def _score_batch(client: Any, query: str, batch: List[Dict[str, Any]]) -> List[float]:
