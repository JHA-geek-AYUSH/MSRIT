...
async def fts_search(db: AsyncSession, q: str, limit: int = 24) -> List[Dict[str, Any]]:
    """Full-text search on authorities table. Returns empty list if fts_doc column doesn't exist."""
    try:
        sql = text(
            """
            select id, title, neutral_cite, reporter_cite,
                   ts_rank(fts_doc, plainto_tsquery('english', :q)) as rank
            from authorities
...
async def trigram_cite_search(db: AsyncSession, cite: str, limit: int = 10) -> List[Dict[str, Any]]:
    """Trigram citation search. Requires pg_trgm extension. Returns empty list if unavailable."""
    try:
        sql = text(
            """
            select id, title, neutral_cite, reporter_cite,
