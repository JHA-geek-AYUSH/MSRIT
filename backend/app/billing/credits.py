...
async def debit_credits(db: AsyncSession, user_id: str, run_id: str | None, 
    ...
    try:
        ...
        await db.execute(text("""
            insert into billing_ledger (id, user_id, run_id, credits_delta, description, created_at) 
            values (gen_random_uuid(), :u, :r, :d, :desc, NOW())
        """), {
            "u": user_id, 
            "r": run_id, 
            "d": -abs(delta),
        ...
        await db.execute(text("""
            insert into billing_accounts (user_id, plan, credits_balance) 
            values (:u, 'free', 0) 
...
async def calculate_and_debit_query_cost(db: AsyncSession, user_id: str, run_id: str | None,
...
async def add_credits(db: AsyncSession, user_id: str, amount: int, description: str = "Credit addition") -> bool:
    ...
    try:
        ...
        await db.execute(text("""
            insert into billing_ledger (id, user_id, run_id, credits_delta, description, created_at) 
            values (gen_random_uuid(), :u, NULL, :d, :desc, NOW())
        """), {
            "u": user_id, 
            "d": amount,
            "desc": description
