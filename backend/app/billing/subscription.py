...
class SubscriptionManager:
    ...
    async def _calculate_proration(self, db: AsyncSession, user_id: str, 
        ...
        days_remaining = max(0, (renews_at - date.today()).days)
        daily_current = current_cost / 30
        daily_new = new_cost / 30
        
        return max(0, (daily_new - daily_current) * days_remaining)
    ...
    def _calculate_refund(self, subscription: Dict[str, Any]) -> float:
        ...
        if not renews_at:
            return 0.0
        days_remaining = max(0, (renews_at - date.today()).days)
        return max(0, (monthly_cost / 30) * days_remaining)
    ...
    async def _record_billing_transaction(self, db: AsyncSession, user_id: str, 
        ...
        sql = text("""
            INSERT INTO billing_ledger (id, user_id, run_id, credits_delta, cost_usd, description, created_at)
            VALUES (gen_random_uuid(), :user_id, :run_id, :credits_delta, :cost_usd, :description, NOW())
        """)
    ...
    def _get_subscription_status(self, renews_at) -> str:
