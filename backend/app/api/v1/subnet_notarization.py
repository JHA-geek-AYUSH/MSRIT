...
async def subnet_notarize(
    ...
    """
    Notarize a research run on the private GemmaChain subnet
    ...
    try:
        ...
        notary_result = await run_in_threadpool(subnet_client.publish_notary, str(run_id), merkle_root_hex)
...
async def subnet_get_notarization(
    ...
    try:
        subnet_client = get_subnet_client()
        subnet_root = await run_in_threadpool(subnet_client.get_notary, str(run_id))
...
async def subnet_get_audit_data(
    ...
    try:
        ...
        subnet_client = get_subnet_client()
        ciphertext = await run_in_threadpool(subnet_client.get_commit, str(run_id))
...
async def _commit_audit_data(commit_id: str, audit_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Commit to subnet
    subnet_client = get_subnet_client()
    return await run_in_threadpool(subnet_client.commit_blob, commit_id, label_hash, ciphertext, data_hash)
