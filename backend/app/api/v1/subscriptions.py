...
async def get_subscription(user=Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    db_user_id = str(await _crud.resolve_db_user_id(db, user["id"], user.get("email", "")))
    
    try:
        manager = SubscriptionManager()
        subscription = await manager.get_user_subscription(db, db_user_id)
        usage = await get_usage_summary(db, db_user_id, days=30)
        ...
        log.error("subscription.get_info_error", user_id=user["id"], error=str(e))
        raise HTTPException(
...
async def upgrade_subscription(
    ...
    import structlog
    log = structlog.get_logger()
    log.info("subscription.upgrade_request", user_id=user["id"], new_plan=request.new_plan, billing_cycle=request.billing_cycle)
    try:
        manager = SubscriptionManager()
        current_sub = await manager.get_user_subscription(db, db_user_id)
        current_plan = current_sub["plan"]
        if current_plan == request.new_plan:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Already subscribed to {request.new_plan} plan")
        result = await manager.upgrade_subscription(db, db_user_id, request.new_plan)
        if not result["success"]:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=result["error"])
        log.info("subscription.upgrade_success", user_id=user["id"], old_plan=current_plan, new_plan=request.new_plan)
        return {"status": "success", "old_plan": current_plan, "new_plan": request.new_plan,
                "effective_date": result.get("renews_at"), "prorated_amount": result.get("proration_amount", 0),
                "new_credits_balance": result.get("credits_added"), "message": f"Successfully upgraded to {request.new_plan} plan"}
    except HTTPException:
        raise
    except Exception as e:
        log.error("subscription.upgrade_error", user_id=user["id"], error=str(e))
...
async def cancel_subscription(user=Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    import structlog
    log = structlog.get_logger()
    log.info("subscription.cancel_request", user_id=user["id"])
    try:
        manager = SubscriptionManager()
        current_sub = await manager.get_user_subscription(db, db_user_id)
        if current_sub["plan"] == "free":
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Already on free plan")
        result = await manager.cancel_subscription(db, db_user_id, immediate=True)
        remaining = await manager._get_credit_balance(db, db_user_id)
        return {"status": "cancelled", "old_plan": current_sub["plan"], "new_plan": "free",
                "effective_date": result.get("effective_date", "immediate"),
                "remaining_credits": remaining, "message": "Subscription cancelled. Downgraded to free tier."}
    except HTTPException:
        raise
    except Exception as e:
        log.error("subscription.cancel_error", user_id=user["id"], error=str(e))
...
async def purchase_credits(
    ...
    import structlog
    log = structlog.get_logger()
    log.info("credits.purchase_request", user_id=user["id"], package=request.package)
    ...
    try:
        ...
        success = await add_credits(db, db_user_id, total_credits, f"Credit purchase - {request.package} package")
        if not success:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to add credits to account")
        new_balance = await get_credit_balance(db, db_user_id)
        log.info("credits.purchase_success", user_id=user["id"], package=request.package,
                credits_added=total_credits, cost_usd=cost_usd, new_balance=new_balance, transaction_id=transaction_id)
        ...
        log.error("credits.purchase_error", user_id=user["id"], error=str(e))
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to process credit purchase")
...
async def get_usage_analytics(
    ...
    try:
        usage = await get_usage_summary(db, db_user_id, days)
        ...
        result = await db.execute(text(daily_query), {"user_id": db_user_id, "days": days})
        ...
        import structlog
        log = structlog.get_logger()
        log.error("subscription.usage_analytics_error", user_id=user["id"], error=str(e))
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve usage analytics")
...
async def check_usage_limits(user=Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        manager = SubscriptionManager()
        query_limit_check = await manager.check_usage_limits(db, db_user_id, "query")
        api_access_check = await manager.check_usage_limits(db, db_user_id, "api_access")
        subscription = await manager.get_user_subscription(db, db_user_id)
        daily_queries = await manager._get_daily_query_count(db, db_user_id)
        ...
        import structlog
        log = structlog.get_logger()
        log.error("subscription.limits_check_error", user_id=user["id"], error=str(e))
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to check usage limits")
...
async def get_billing_history(
    ...
    try:
        result = await db.execute(text("""
            SELECT id, run_id, credits_delta, cost_usd, created_at,
                CASE WHEN credits_delta > 0 THEN 'credit_purchase'
                     WHEN run_id IS NOT NULL THEN 'query_usage'
                     ELSE 'other' END as transaction_type
            FROM billing_ledger WHERE user_id = :user_id
            ORDER BY created_at DESC LIMIT :limit
        """), {"user_id": db_user_id, "limit": limit})
        ...
        import structlog
        log = structlog.get_logger()
        log.error("subscription.billing_history_error", user_id=user["id"], error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
