...
async def get_user_profile(user=Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    # resolve_db_user_id auto-creates user + billing account on first login
    db_user_id = await crud.resolve_db_user_id(db, user["id"], user.get("email", ""))
    db_user = await crud.get_user_by_id(db, db_user_id)
    if not db_user:
...
async def update_user_profile(
    ...
    user_id = await crud.resolve_db_user_id(db, user["id"], user.get("email", ""))
    ...
    if update_data.role is not None:
        ...
        if update_data.role not in allowed_roles:
            raise HTTPException(
                ...
                detail=f"Invalid role '{update_data.role}'. Must be one of: {', '.join(sorted(allowed_roles))}",
            )
