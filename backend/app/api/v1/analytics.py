...
async def get_usage_metrics(
    ...
    try:
        ...
        # Fallback to empty metrics if query fails (e.g. on SQLite)
        async def _fallback():
            return UsageMetrics(
...
async def get_query_analytics(
    ...
    try:
        ...
        import structlog
        log = structlog.get_logger()
        log.warning("analytics.query_analytics_error", user_id=user_id, error=str(e))
        return QueryAnalytics(
            total_queries=0,
            queries_by_mode={},
            queries_by_day=[],
...
async def get_cost_analytics(
    ...
    try:
        ...
        import structlog
        log = structlog.get_logger()
        log.warning("analytics.cost_analytics_error", user_id=user_id, error=str(e))
        return CostAnalytics(
...
async def get_performance_metrics(
    ...
    try:
        ...
        import structlog
        log = structlog.get_logger()
        log.warning("analytics.performance_metrics_error", user_id=user_id, error=str(e))
        return PerformanceMetrics(
            average_response_time=0.0,
            query_success_rate=0.0,
            verification_pass_rate=0.0,
...
async def get_content_metrics(
    ...
    try:
        ...
        # Queries with citations — uses simple IS NOT NULL check (works in both PostgreSQL and SQLite)
        try:
            citation_sql = """
                SELECT COUNT(*) 
                FROM runs r
                ...
                WHERE m.user_id = :user_id 
                AND r.created_at >= :period_start
                AND r.retrieval_set_json IS NOT NULL
            """
        ...
        import structlog
        log = structlog.get_logger()
        log.warning("analytics.content_metrics_error", user_id=user_id, error=str(e))
        return ContentMetrics(
...
async def get_dashboard_summary(user=Depends(current_user), db: AsyncSession = Depends(get_db)):
    """Get summary analytics for dashboard — resolves Clerk ID to DB UUID."""
    import structlog
    log = structlog.get_logger()

    # ── Resolve Clerk string → DB UUID ──────────────────────────────────────
    from app.db import crud as _crud
    clerk_id: str = user["id"]
    db_user = await _crud.get_user_by_clerk_id(db, clerk_id)
    if not db_user:
        db_user = await _crud.create_user(
            db, clerk_id=clerk_id, email=user.get("email") or f"{clerk_id}@gemmaFin.local", role="lawyer"
        )
        await _crud.get_or_create_billing_account(db, db_user.id)

    db_user_id = str(db_user.id)   # UUID string — safe for SQL UUID columns
    # ────────────────────────────────────────────────────────────────────────
    ...
    try:
        ...
        quick_metrics_sql = """
            WITH recent_activity AS (
                SELECT
                    ...
                    COALESCE(SUM(
                        CASE WHEN bl.credits_delta < 0
                             THEN ABS(bl.credits_delta) ELSE 0 END
                    ), 0) AS credits_spent
                FROM matters m
                LEFT JOIN queries q
                       ON q.matter_id = m.id
            ...
            current_balance AS (
                SELECT COALESCE(credits_balance, 0) AS balance, plan
            ...
            SELECT
                ...
                COALESCE(cb.plan, 'free')
            FROM recent_activity ra
            LEFT JOIN current_balance cb ON TRUE
        """
        uid_param = db_user_id  # Pass UUID directly, no ::uuid cast needed
        result = await db.execute(text(quick_metrics_sql), {"uid": uid_param, "cutoff": cutoff})
        row = result.fetchone()

        total_matters_result = await db.execute(
            text("SELECT COUNT(*) FROM matters WHERE user_id = :uid"),
            {"uid": uid_param},
        )
        total_matters = total_matters_result.scalar() or 0

        if not row:
            return {
                "recent_activity": {"queries_last_30_days": 0, "documents_uploaded": 0, "successful_runs": 0, "credits_spent": 0},
                "account_status": {"current_balance": 0, "plan": "free", "can_make_queries": False},
                "quick_stats": {"total_matters": total_matters, "average_query_cost": 0, "success_rate": 0},
            }

        queries_count = int(row[0] or 0)
        credits_spent = int(row[3] or 0)
        ...
        return {
            "recent_activity": {
                ...
                "documents_uploaded": int(row[1] or 0),
            ...
            "account_status": {
                ...
                "plan": plan,
                "can_make_queries": balance > 0,
            },
            "quick_stats": {
                "total_matters": total_matters,
                "average_query_cost": credits_spent / max(queries_count, 1),
                "success_rate": int(row[2] or 0) / max(queries_count, 1),
            },
        }

    except Exception as e:
        log.error("analytics.dashboard_summary_error", user_id=clerk_id, error=str(e))
        # Return default response instead of 500 — dashboard should never crash
        return {
            "recent_activity": {"queries_last_30_days": 0, "documents_uploaded": 0, "successful_runs": 0, "credits_spent": 0},
            "account_status": {"current_balance": 0, "plan": "free", "can_make_queries": False},
            "quick_stats": {"total_matters": 0, "average_query_cost": 0, "success_rate": 0},
        }
