// GemmaFinOS Phase 2 - Network Configuration
// Manages configuration for the private GemmaChain Subnet
...
export const NETWORKS: Record<string, NetworkConfig> = {
  // GemmaFinOS Private Subnet (Phase 2 Production)
  gemmaFin_production: {
    id: 43210,
    name: 'GemmaFinOS Private Subnet',
    displayName: 'GemmaFinOS Private Network',
    rpcUrl: 'https://gemmaFin-rpc.internal', // VPN-only access
  ...
  // GemmaChain Fuji Testnet (Legacy/Development)
  gemmachain_fuji: {
    id: 43113,
    name: 'GemmaChain Fuji Testnet',
    displayName: 'GemmaChain Fuji',
    rpcUrl: 'https://api.gemmachain-test.network/ext/bc/C/rpc',
...
export function getCurrentNetwork(): NetworkConfig {
  const networkEnv = process.env.NEXT_PUBLIC_NETWORK || 'gemmaFin_production';

  // Default to production network
  return NETWORKS[networkEnv] || NETWORKS.gemmaFin_production;
}
...
export function getContractAddresses() {
  ...
  const networkKey = Object.keys(NETWORKS).find(key => NETWORKS[key].id === network.id) || 'gemmaFin_production';
...
export function getNetworkStatus(networkKey: string): 'active' | 'maintenance' | 'deprecated' {
  switch (networkKey) {
    case 'gemmaFin_production':
      return 'active';
    case 'gemmachain_fuji':
...
export function getEstimatedCost(operation: keyof typeof GAS_ESTIMATES): string {
  ...
  if (network.id === 43210) { // GemmaFinOS Private Subnet
