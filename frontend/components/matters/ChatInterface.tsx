...
export function ChatInterface({ matterId, disabled, onEvidenceSelect, onRunComplete }: ChatInterfaceProps) {
  ...
  const loadChatHistory = async () => {
    try {
      const response = await apiClient.getConversationHistory(matterId);
      if (response.data?.messages && response.data.messages.length > 0) {
        const loaded: ChatMessage[] = response.data.messages.map((msg: any, i: number) => ({
          id: msg.run_id || `hist-${i}`,
    ...
    try {
      ...
      if (response.data) {
        const aiMessage: ChatMessage = {
          ...
          timestamp: new Date().toISOString(),
          evidence: response.data.citations?.map((citation: any) => ({
            ...
            paragraphs: citation.para_ids.map((id: unknown) => String(id)),
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-brown-500 text-sm max-w-md">
                    ...
                    or get help with document drafting. GemmaFinOS&apos;s AI will search through legal authorities
