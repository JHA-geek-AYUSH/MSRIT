...
export function MatterWorkspace({
  ...
  const loadMatterData = async () => {
    try {
      ...
      if (documentsResponse.data) {
        // Backend returns { data: [...] } — unwrap the nested data field
        const docsArray = Array.isArray(documentsResponse.data)
          ? documentsResponse.data
          : documentsResponse.data.data || [];
        setDocuments(docsArray);

        // Check if any documents are still processing
        const hasProcessingDocs = docsArray.some(
          (doc: any) =>
            doc.status === "processing" || doc.status === "pending"
        );
        if (hasProcessingDocs && !isNewlyCreated) {
          setIsProcessing(true);
          // Poll for updates — stop after 10 attempts (50 seconds)
          let pollAttempts = 0;
          ...
          const pollTimer = setInterval(async () => {
            ...
            if (updatedDocs.data) {
              const newDocs = Array.isArray(updatedDocs.data)
                ? updatedDocs.data
                : updatedDocs.data.data || [];
              setDocuments(newDocs);
              const stillProcessing = newDocs.some(
                (doc: any) =>
                  doc.status === "processing" || doc.status === "pending"
              );
              if (!stillProcessing || pollAttempts >= MAX_POLL_ATTEMPTS) {
                setIsProcessing(false);
  ...
  const handleExport = async (format: 'docx' | 'pdf' | 'json' = 'docx') => {
    if (!currentRunId) {
      toast({ title: 'No run to export', description: 'Make a research query first.', variant: 'destructive' });
      return;
    }
    try {
      const res = await apiClient.exportRun(currentRunId, format);
      if (res.error) {
      ...
      if (res.data?.url) {
        ...
        toast({ title: 'Export ready', description: `${format.toUpperCase()} download started.` });
      }
    } catch (err) {
